...
ALLOW_OTP_CONSOLE_FALLBACK=false
TWOFACTOR_API_KEY=your_2factor_api_key
```

Citizen login/register OTPs are delivered by a 2Factor voice OTP call. Without live 2Factor access, the backend can print the OTP in the server console for local testing only. In production, keep `ALLOW_OTP_CONSOLE_FALLBACK=false`.
...
| OTP | 2Factor.in Voice OTP |

Read [DEPLOYMENT_GUIDE.md](DEPLOYMENT_GUIDE.md) for the exact Render and Vercel settings.
